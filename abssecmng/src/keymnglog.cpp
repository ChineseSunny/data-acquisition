...
int   LogHelper::KeyMngLevel[5] = {IC_NO_LOG_LEVEL, IC_DEBUG_LEVEL, IC_INFO_LEVEL, IC_WARNING_LEVEL, IC_ERROR_LEVEL};

//Level的名称
char  LogHelper::ICLevelName[5][10] = {"NOLOG", "DEBUG", "INFO", "WARNING", "ERROR"};

int LogHelper::ITCAST_Error_GetCurTime(char* strTime)
{
...
int LogHelper::ITCAST_Error_OpenFile(int* pf)
{
	char	fileName[1024];
...
void LogHelper::ITCAST_Error_Core(const char *file, int line, int level, int status, const char *fmt, va_list args)
{
...
void LogHelper::KeyMng_Log(const char *file, int line, int level, int status, const char *fmt, ...)
{
    va_list args;
