...
int KeyMng_ShmOp::KeyMng_ShmWrite(int shmhdl, int maxnodenum, NodeSHMInfo *pNodeInfo)
{
	...
	for (i=0; i<maxnodenum; i++)
	{
		...
		{
			//KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[3], ret,"系统检测到网点密钥已经存在 覆盖clientId:%s serverid:%s", pNodeInfo->clientId,pNodeInfo->serverId);
	...
	for (i=0; i<maxnodenum; i++)
	{
		...
		{
			//KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[3], ret,"系统检测到空节点 写入网点信息clientId:%s serverid:%s", pNodeInfo->clientId,pNodeInfo->serverId);
			memcpy(pNode, pNodeInfo, sizeof(NodeSHMInfo));
			goto End;
		}
	}
	...
	{
		ret = 102;
		//KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"系统检测到网点节点已满错误 clientId:%s serverid:%s", pNodeInfo->clientId,pNodeInfo->serverId);
		goto End;
	}
...
int KeyMng_ShmOp::KeyMng_ShmRead(int shmhdl, char *clientId, char *serverId,  int maxnodenum, NodeSHMInfo *pNodeInfo)
{
	...
	{
		ret = 102;
		//KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"系统 没有找到网点节点错误 clientId:%s serverid:%s", pNodeInfo->clientId,pNodeInfo->serverId);
		goto End;
	}
