...
int  KeyMngServerOp_LiSi::MngServer_InitInfo(MngServer_Info *svrInfo)
{
...
//服务器端 应答流程
int  KeyMngServerOp_LiSi::MngServer_Agree(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	...
	if (ret != 0)
	{
		LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[4], ret,"func KeyMngsvr_DBOp_WriteSecKey() err:%d \n", ret);
		goto End;
	}
...
//和客户端的数据 进行比较
//ok -- 0   err
int  KeyMngServerOp_LiSi::MngServer_Check(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	/* 
	int					 i = 0, ret = 0;
...
int KeyMngServerOp_LiSi::MngServer_Revoke(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	return 0;
}

int KeyMngServerOp_LiSi::MngServer_view(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	return 0;
}

int KeyMngServerOp_LiSi::MngServer_Quit(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	return 0;
}